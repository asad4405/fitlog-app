...
interface WorkoutsTypeProps {
    workout: WorkoutsType;
}
const Workout = ({ workout }: WorkoutsTypeProps) => {
    return (
        <Link href={`/workouts/${workout.id}`}>
            <div className="bg-[#15171D] border border-gray-800/80 rounded-2xl overflow-hidden flex flex-col justify-between">
                    ...
                    <div className="space-y-2">
                        <div className="flex items-center gap-2 flex-wrap">
                            {workout.muscleGroups?.map((group, index) => (
                                <span
                                    key={index}
                                    className="bg-[#C2F800] text-black font-extrabold text-[10px] px-2.5 py-0.5 rounded-full uppercase tracking-wider"
                                >
                                    {group}
                                </span>
                            ))}
